...
function setupSocketHandlers(io, db, opts = {}) {
  ...
  function emitOnlineUsers(code) {
    ...
    const statusMap = {};
    try {
      const statusRows = db.prepare('SELECT id, status, status_text, avatar, avatar_shape, border, border_transform, is_guest FROM users').all();
      statusRows.forEach(r => { statusMap[r.id] = { status: r.status || 'online', statusText: r.status_text || '', avatar: r.avatar || null, avatarShape: r.avatar_shape || 'circle', border: r.border || null, borderTransform: parseBorderTransform(r.border_transform), isGuest: !!r.is_guest }; });
  ...
  io.use((socket, next) => {
    ...
    try {
      ...
      const uRow = db.prepare('SELECT display_name, is_admin, username, avatar, avatar_shape, border, border_transform, password_version, is_guest, created_at, oidc_subject FROM users WHERE id = ?').get(user.id);
